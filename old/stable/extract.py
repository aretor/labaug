    ...
    for exp in range(cfg.NUM_EXPS):
        for set_ in ('train', 'test'):
            ...
            for net_model in net_models:
                print('exp {}: {}, {}, {} classes'.format(exp, set_, net_model, nr_classes))
                ft_model = get_finetune_model(net_model, nr_classes)
                set_loader = prepare_loader(set_pname, dset_src, stats,
                                            batch_size, False)

                fc7_features, labels, net, fnames = fe.extract_features_train(
                    ft_model, set_loader, net_model)
                ...
                with open(osp.join(pickle_dir,
                                   net_model + '_' + str(exp) + '.pickle'),
                          'wb') as f:
